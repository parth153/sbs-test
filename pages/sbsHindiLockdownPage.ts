...
export class SbsHindiLockDownPage {
  ...
  async verifyPlayPauseControl(playPauseOption: string) {
      ...
      const classToCheck = playPauseOption === "Play" ? this.audioPlayerClasses.isMediaPlaying : this.audioPlayerClasses.isMediaPaused;
      ...
      const afterClickTimeElapsed = await I.grabTextFrom(this.audioPlayerFields.audioPlayerTimeElapsed);
      I.wait(2);
      const afterWaitTimeElapsed = await I.grabTextFrom(this.audioPlayerFields.audioPlayerTimeElapsed);
      ...
      const classesAudioArticle = await I.grabAttributeFrom(this.audioPlayerFields.audioArticle, "class");
      I.assertContain(classesAudioArticle, classToCheck);
  }

  async verifyMuteUnmuteControl(muteUnmuteOption) {
      // class to be verified
      const classToCheck = this.audioPlayerClasses.isMuted;
  ...
  async verifyScrubOnProgressBar(seconds: number, forwardRewindOption: string) {
      ...
      const elapsedTimeBeforeClick = await I.grabTextFrom(this.audioPlayerFields.audioPlayerTimeElapsed);
      const styleWidthBeforeClick = await I.grabAttributeFrom(this.audioPlayerFields.audioPlayerProgressBar, "style");
      ...
      const elapsedTimeAfterClick = await I.grabTextFrom(this.audioPlayerFields.audioPlayerTimeElapsed);
      const styleWidthAfterClick = await I.grabAttributeFrom(this.audioPlayerFields.audioPlayerProgressBar, "style");
